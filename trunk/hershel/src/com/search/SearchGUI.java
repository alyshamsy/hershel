...
public class SearchGUI extends Thread implements GUI, IRemote {
	...
	private com.search.gui.SearchWindow searchWindow;

	public SearchGUI() throws IOException {
		port = 10000;
		client = new NetworkSearchClient(initialId, 10000);
		ms = new MonitorService(10001, this, new InetSocketAddress(InetAddress
		...
		searchWindow = new com.search.gui.SearchWindow(client, this, listener);
	}
	...
	public SearchGUI(int port) throws IOException {
		...
		ms = new MonitorService(port + 1, this, new InetSocketAddress(
	...
	public static SearchResult createSearchResult(String fileName) {
		...
		// SearchId.fromHex("4dd974e5ddca2736619a83ec4ca9e3846c7ac54f");
		SearchId fileNameHash = new SearchId(SHA1Utils.getSHA1Digest(fileName
				.getBytes()));
		...
		return new SearchResult(fileName, fileNameHash, fileHash, chunkHashes,
				f.length(), 16 * 1024, peers);
	}
	...
	public InetSocketAddress getLocalAddress() {
		try {
			return new InetSocketAddress(InetAddress.getByName("localhost"),
					port);
	...
	public InetSocketAddress[] getPeers(int layer) {
		ArrayList<ArrayList<NodeState>> rt = client.getHandler().routingTable()
				.getRoutingTable();
		ArrayList<InetSocketAddress> peers = new ArrayList<InetSocketAddress>();
		for (int i = 0; i < 160; i++) {
		...
		while (true) {
			...
			if (command[0].equalsIgnoreCase("search")) {
				...
				try {
					...
					client.updateDatabase(result.fileNameHash,
							new InetSocketAddress("localhost", port + 6000));
					String outputFileName = (command[2] != null) ? command[2]
							: "output.txt";
					listener.download(result, outputFileName,
							fileTransferServer);
				} catch (Exception ex) {
				...
				searchWindow.setTitle(command[1] + ":" + command[2]);
